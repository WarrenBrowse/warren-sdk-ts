 ...
 * its own local host. The matcher is shared by the Chromium PAC script and the
 * Firefox `proxy.onRequest` path so both platforms decide identically.
 */
...
/** The Firefox proxy answer for a request that must fail closed: a proxy on
 * loopback port 1, which nothing listens on, so the request stalls instead of
 * escaping to a direct route. A bare `{ type: 'direct' }` would leak. */
export const FAIL_CLOSED_PROXY: readonly { type: 'https'; host: string; port: number }[] = [
...
export function shouldTunnelHost(host: string, config: SplitTunnelConfig): boolean {
  // Loopback is never tunneled: the host's own listeners live there.
...
/** Builds a PAC script routing tunneled hosts to the HTTP listener, others direct. */
function buildPacScript(http: string, config: SplitTunnelConfig): string {
  ...
  return tunnel ? 'PROXY ${http}' : 'DIRECT';
}`;
}

/**
 * Builds the `chrome.proxy.settings` value for a Chromium browser, pointed at
 * the host's HTTP listener: Chromium cannot authenticate to a SOCKS5 proxy,
 * and answers the HTTP listener's `407` through `webRequest.onAuthRequired`.
 */
export function buildChromiumProxyValue(http: string, config: SplitTunnelConfig): unknown {
  ...
  if (config.mode === 'only') {
    return {
      mode: 'pac_script',
      pacScript: { data: buildPacScript(http, config), mandatory: true },
    };
  }
  ...
  return {
    mode: 'fixed_servers',
    rules: { singleProxy: { scheme: 'http', host, port }, bypassList },
  };
}

/**
 * Builds the `browser.proxy.settings` value for Firefox. Only `all`/`bypass`
 * are expressible here (via `passthrough`). It cannot carry the listener's
 * credentials, so it only holds the browser on the tunnel when no
 * `proxy.onRequest` handler answers: a request reaching it is refused, never
 * sent direct.
 */
